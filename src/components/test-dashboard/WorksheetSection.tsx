...
import { getChaptersWithQuestions, getWorksheetQuestions } from "@/data/testQuestions";
import QuizEngine from "./QuizEngine";
import TestResults from "./TestResults";
import { TestQuestion, TestResult } from "@/types/testDashboard";

type WorksheetState = "subjects" | "chapters" | "quiz" | "results";

const WorksheetSection = ({ onBack }: { onBack: () => void }) => {
  const [state, setState] = useState<WorksheetState>("subjects");
  const [selectedSubject, setSelectedSubject] = useState("");
  const [selectedChapter, setSelectedChapter] = useState("");
  const [questions, setQuestions] = useState<TestQuestion[]>([]);
  const [results, setResults] = useState<TestResult[]>([]);

  const startWorksheet = (chapter: string) => {
    setSelectedChapter(chapter);
    const qs = getWorksheetQuestions(chapter, 25);
    setQuestions(qs);
    setState("quiz");
  };
  ...
  if (state === "quiz" && questions.length > 0) {
    return (
      <QuizEngine
        questions={questions}
        timeMinutes={30}
        title={`Worksheet: ${selectedChapter}`}
  ...
  if (state === "results") {
    return (
      <TestResults
        ...
        results={results}
        title={`Worksheet: ${selectedChapter}`}
        onRetake={() => startWorksheet(selectedChapter)}
        onBack={() => setState("chapters")}
      />
    );
  }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="font-display font-bold text-lg text-foreground">{selectedSubject} - Worksheets</h2>
        </div>
        <div className="space-y-2.5">
          {chapters.map((ch, i) => (
            <motion.button
              ...
              onClick={() => startWorksheet(ch.chapter)}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">25 MCQs • Instant feedback</p>
              </div>
              <Play className="w-4 h-4 text-accent shrink-0" />
            </motion.button>
          ))}
          {chapters.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-8">No worksheets available yet.</p>
          )}
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="font-display font-bold text-lg text-foreground">Worksheets</h2>
          <p className="text-xs text-muted-foreground">Chapter-wise • 25 MCQs • Instant answers</p>
        </div>
      </div>
