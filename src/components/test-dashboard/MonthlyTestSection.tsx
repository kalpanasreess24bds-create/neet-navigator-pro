...
import { getMonthlyTestQuestions } from "@/data/testQuestions";
import QuizEngine from "./QuizEngine";
import TestResults from "./TestResults";
import { TestQuestion, TestResult } from "@/types/testDashboard";

type MonthlyState = "intro" | "quiz" | "results";

const MonthlyTestSection = ({ onBack }: { onBack: () => void }) => {
  const [state, setState] = useState<MonthlyState>("intro");
  const [questions, setQuestions] = useState<TestQuestion[]>([]);
  const [results, setResults] = useState<TestResult[]>([]);

  const startTest = () => {
    const qs = getMonthlyTestQuestions(45);
    setQuestions(qs);
    setState("quiz");
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="font-display font-bold text-lg text-foreground">Monthly Test</h2>
      </div>
      ...
      <div className="elevated-card rounded-2xl p-6 text-center">
        ...
        <h3 className="font-display font-bold text-xl text-card-foreground">Full NEET Mock</h3>
        <p className="text-sm text-muted-foreground mt-2">Complete NEET pattern • 180 questions • 3 hours</p>
        ...
        <div className="bg-secondary rounded-xl p-3 mt-4 text-left">
          <p className="text-xs font-semibold text-card-foreground mb-1">Detailed Report Includes:</p>
          <ul className="text-[10px] text-muted-foreground space-y-0.5">
            <li>• Subject-wise score breakdown</li>
            <li>• Weak topic identification</li>
            <li>• Accuracy percentage per subject</li>
            <li>• Time spent per subject</li>
          </ul>
        </div>
        ...
        <Button onClick={startTest} className="w-full mt-6 gradient-primary text-primary-foreground border-0 rounded-xl h-12 font-semibold gap-2">
          Start Monthly Test <ArrowRight className="w-4 h-4" />
        </Button>
      </div>
    </motion.div>
  );
...
export default MonthlyTestSection;
